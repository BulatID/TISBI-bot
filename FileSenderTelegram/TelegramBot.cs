...
namespace TISBIHelperBot
{
    public class TelegramBot : BotController
    {
        ...
        /*[Action("/say", "Спросить о чем-то")]
        public void sayMe(string message)
        {
            var chatGPT = new ChatGPT(message);

            Context.Bot.Client.SendTextMessageAsync(ChatId, chatGPT.sendText(message));
        }*/

        [Action]
        public void kButtonDelete()
        {
            //Context.Bot.Client.SendTextMessageAsync(ChatId, "Хорошо", replyMarkup: new ReplyKeyboardRemove());
        }
        ...
        public void Start()
        {
            ...
            PushL("— Помогаю найти тебе работу 💰");
            PushLL("— Также у нас есть глобальный чат студентов 💬");
        ...
        {
            PushL("Ваш профиль");
            PushL("Уникальный ID: " + ChatId);
            PushLL("Cтатус: ");
            PushLL("Данные от системы управления вузом");
            PushL("Логин: ");
            PushL("Пароль: ");

            RowButton("Оповещение о мероприятиях в ВУЗе: ", Q(MainMenu));
            RowButton("Показывать вакансии: ", Q(MainMenu));
            RowButton("Показывать партнерские материалы: ", Q(MainMenu));
            RowButton("Удалить профиль ❌", Q(deleteUserData));
